...
def history():
    ...
    stocks = db.execute(
        "SELECT symbol, price, type, date, shares FROM transactions WHERE user_id = ?",
        userID,
    )
...
def quote():
    ...
    # POST method receives the symbol and redirects to "/quoted" to show the stats of the stock
        ...
        return render_template(
            "quoted.html", stockQuote=stockQuote, price=usd(stockQuote["price"])
        )
...
def sell():
    ...
    # POST method, use the received info from the form to sell the user's stock and update accordingly
        ...
        cash = (
            db.execute("SELECT cash FROM users WHERE id = ?", userID)[0]["cash"] + price
        )
        ...
        if float(numShares) == selectedStockShares:
            db.execute(
                "DELETE FROM portfolio WHERE symbol = ? AND user_id = ?", symbol, userID
            )
