...
def index():
    ...
    stocks = db.execute(
        "SELECT symbol, SUM(shares) AS shares FROM portfolio WHERE user_id = ? GROUP BY symbol;",
        currentUserID,
    )
...
def buy():
    ...
    # POST method submits the user's request to buy x shares of a stock
        ...
        # Get price and transaction total
        price = stock["price"]
        total = price * float(shares)

        # Update transactions table in SQL
        db.execute(
            "INSERT INTO transactions (user_id, symbol, shares, price, total, date, type) VALUES (?, ?, ?, ?, ?, ?, ?)",
            currentUserID,
            symbol,
            shares,
            price,
        ...
        if symbol in ownedStocks:
            selectedStockShares = db.execute(
                "SELECT SUM(shares) AS shares FROM portfolio WHERE symbol = ? AND user_id = ?",
                symbol,
                currentUserID,
            )[0]["shares"]
            db.execute(
                "UPDATE portfolio SET shares = ? WHERE user_id = ? AND symbol = ?",
                (selectedStockShares + float(shares)),
                currentUserID,
                symbol,
            )
...
def history():
    ...
    stocks = db.execute(
        "SELECT symbol, price, total, type, date, shares FROM transactions WHERE user_id = ?",
        userID,
    )
...
@login_required  # sell = login_required(sell)
def sell():
    ...
    # POST method, use the received info from the form to sell the user's stock and update accordingly
        ...
        # Get the price the stock sold for and total of the transaction
        price = lookup(symbol)["price"]  # * float(numShares) // Only do this if you want the total
        total = price * float(numShares)

        # Get the user's cash (cash + price just sold) balance from SQL
        cash = db.execute("SELECT cash FROM users WHERE id = ?", userID)[0]["cash"] + total
        ...
        if float(numShares) == selectedStockShares:
            ...
            db.execute(
                "UPDATE portfolio SET shares = ? WHERE user_id = ? AND symbol = ?",
                (selectedStockShares - float(numShares)),
                userID,
                symbol,
            )
