#include <stdio.h>
#include <stdlib.h>

// Check if the given number 'X' is odd or even
#define ODD(X) ((X)&01)

// Determine if the Nth bit of the given number 'X' is on (1) or not (0)
#define BITON(X, N) (((X) & (1U << (N))) != 0)

// Determine if the bits from the given number 'X' in the range given from 'S'(Start) to 'E'(End) are on
#define ALLON(X, S, E) (((X) & (((1U << ((E) + 1)) - 1) ^ ((1U << (S)) - 1))) == (((1U << ((E) + 1)) - 1) ^ ((1U << (S)) - 1)))

//-----------------------------------------------------------------------------
int main(void)
{

    unsigned int U1, BitNumber, Start, End;

    printf("Enter an integer : ");
    scanf("%ud", &U1);
    printf("%u is %s\n", U1, ODD(U1) ? "odd" : "even");

    printf("Enter an integer and a bit number : ");
    scanf("%u %d", &U1, &BitNumber);
    printf("%u has bit %d %s\n", U1, BitNumber, BITON(U1, BitNumber) ? "on" : "off");

    printf("Enter an integer, start and end bit numbers : ");
    scanf("%u %u %u", &U1, &Start, &End);
    printf("%u has %s those bits on\n", U1, ALLON(U1, Start, End) ? "all" : "not all");

    return (EXIT_SUCCESS);
}
//-----------------------------------------------------------------------------